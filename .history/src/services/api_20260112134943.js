...
export const fetchSmartMovieData = async (lang, targetGenreId = null, shownIds = []) => {
  ...
  try {
    ...
    const videosRes = await fetch(`${BASE_URL}/movie/${movie.id}/videos?api_key=${TMDB_API_KEY}&language=${apiLang}`); // Önce kendi dilinde ara
    let videosJson = await videosRes.json();

    // Eğer kendi dilinde video yoksa, İngilizce fragmanları dene (Boş gelmesin)
