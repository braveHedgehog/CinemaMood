...
export default function App() {
  ...
  const handleFetch = async () => {
    ...
    try {
        ...
        // shownIds listesini API'ye gönderiyoruz
        const data = await fetchSmartMovieData(lang, targetGenre, shownIds);
        
        if (data) {
            const processed = processMovieData(data);
            setResult(processed);
            
            // YENİ: Gösterilen filmi hafızaya ekle
            setShownIds(prev => [...prev, processed.id]);
  ...
  const handleGenreFetch = async (genreId) => {
    ...
    try {
        // shownIds listesini buraya da gönderiyoruz
        const data = await fetchSmartMovieData(lang, genreId, shownIds);
