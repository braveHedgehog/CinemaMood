...
// Importlarımız (Kendi yazdığımız modüller)
...
import DetailScreen from './src/screens/DetailScreen'; // (Bunu aşağıda yazacağız, mantık olarak Home ile aynı)
...
export default function App() {
  ...
  const processMovieData = (rawData) => {
    // Burada API'den gelen veriyi işleyip UI'ın anlayacağı formata çeviriyoruz
    const { movie, credits, providers } = rawData;
    let platformName = T.noPlatform;
    
    // Platform belirleme mantığı (kısaltıldı)
    if (providers) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        // Detail Screen'i de ayırırsan burası çok daha temiz olur
        <DetailScreen 
            result={result}
            onBack={() => setStep(1)}
            onAgain={handleFetch}
